...
./bin/ycsb run mongodb -threads ${m} -s -P workloads/workloada -P /root/workload-mongo-perf-run  -p exportmeasurementsinterval=1000 -p measurementtype=timeseries -p timeseries.granularity=100 -p mongodb.readPreference=primary -p mongodb.url=mongodb://${MONGO_ADDR}:27017/ycsb?w=0 -cp . | grep -v DEBUG |& tee /root/${MONGO_ADDR}-$2-${m}-threads.run
