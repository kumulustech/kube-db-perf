...
/root/ycsb-0.7.0/bin/ycsb load jdbc -P workloads/workloada -p db.driver=com.mysql.jdbc.Driver -p db.url=jdbc:mysql://${MONGO_ADDR}:3306/ycsb -p db.user=root -p db.passwd=admin -threads 128 -s -P /root/kube-mongo-scale/workload-mysql-perf-load -cp /usr/share/java/mysql-connector-java-5.1.28.jar:/usr/share/java:/root:.
for m in 16 32 64; do
pid=`ssh sql-minion-$[${offset}+1] "./iostat_capture.sh $n $m"`
...
ssh sql-minion-$[${offset}+1] "kill $pid"
done
